      ...
      >
        ...
        {!item.submenu && (
          <Link
            ...
            // style={{ backgroundColor: this.props.history.location.pathname === `/${item.page || ""}` ? "red" : "" }}
          >
            <MenuItemInner item={item} parentItem={parentItem} currentUrl={currentUrl} />
          </Link>
        )}
        ...
        {item.submenu && (
          <div className={`kt-menu__submenu ${this.getItemMenuSubmenuClass(item)}`} style={{ width: item.submenu.width }}>
            <span
