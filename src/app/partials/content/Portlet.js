...
export const PortletHeader = forwardRef(({ icon, title, toolbar, className, sticky = false, labelRef, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className="kt-portlet__head" style={!sticky ? undefined : { top, position: "sticky", backgroundColor: "#fff" }}>
      {title != null && (
        <div ref={labelRef} className={clsx("kt-portlet__head-label", className)}>
          {icon}

          {/* Wrap string and fragments in PortletHeaderTitle */
          typeof title === "string" || isFragment(title) ? <PortletHeaderTitle>{title}</PortletHeaderTitle> : title}
        </div>
      )}
